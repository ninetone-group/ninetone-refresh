...
- **Image components use `.fm-img-frame` + `.fm-img`** for the shimmer skeleton + fade-in pattern. Defined in [src/styles/global.css](src/styles/global.css). Wire `onload="this.classList.add('is-loaded'); this.closest('.fm-img-frame')?.classList.add('is-ready')"` on the `<img>`. Base.astro's `astro:before-swap` handler relies on exactly this selector and these two classes: it pre-marks images already shown on screen (same `src`) as loaded in the incoming document, so they paint on the first frame instead of shimmering again after a client-router swap. Keep the class names and `img.fm-img` selector in any new image component.
...
- **Secrets are read from `process.env` only, through [src/lib/env.ts](src/lib/env.ts). Never write `import.meta.env.<SECRET>`, and never read `import.meta.env` by a dynamic name.** Astro replaces `import.meta.env.X` with the literal value at build time, and on the cf target the adapter first copies `.dev.vars` into `process.env`, so blanking variables on the command line does not help; a dynamic read (`env[name]`) inlines the whole environment. Both shipped real credentials inside the Worker, overriding the Cloudflare secrets (found 2026-09-19, v0.2.5.6). `astro.config.mjs` copies `.env` onto `process.env` for dev and the static build. `npm run build:cf` ends with `scripts/check-no-baked-secrets.mjs`, which fails the build (and therefore `deploy:cf`) if any local secret value is in `dist/`. A new non-secret setting for the Worker goes in `vars` in [wrangler.jsonc](wrangler.jsonc); a new secret goes in with `wrangler secret put`.
